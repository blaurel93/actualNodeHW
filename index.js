    ...
    .then(function (data) {
        ...
        axios.get(queryUrl).then(function (res) {
            ...
            axios.get(queryUrl1).then(function (resa) {
                ...
                var chain = [location, realLoc, reposs, followers, following, photo, starred, name, blog, gitSite, colar]
                // console.log(chain);
                // console.log(chain);
                var pagegen = generateHTML(chain);
                // console.log(queryUrl1.resa.length);

                (async () => {
                    const browser = await puppeteer.launch();
                    const page = await browser.newPage();
                    await page.setContent(pagegen, { waitUntil: 'networkidle2' });
                    await page.pdf({
                        path: 'geneed.pdf',
                        format: 'A4',
